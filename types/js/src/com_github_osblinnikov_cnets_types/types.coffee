...
s.com_github_osblinnikov_cnets_types =
  create: ()->
    that = this
    
    #dummy
#[[[end]]] (checksum: bc619e155ecb3f492b023166fd31b3ad) (573bd6781a129adf050691d6429e4ccd)

s.com_github_osblinnikov_cnets_types.bufferReadData = ->
  self = this
...
s.com_github_osblinnikov_cnets_types.bufferKernelParams = (target, grid_id, additionalData)->
  self = this
  self.target = target
...
s.com_github_osblinnikov_cnets_types.MessagePort = (anotherPort)->
  slf = this
...
if s.MessageChannel
  s.com_github_osblinnikov_cnets_types.MessageChannel = s.MessageChannel
else if !s.com_github_osblinnikov_cnets_types.MessageChannel
  s.com_github_osblinnikov_cnets_types.MessageChannelArtificial = true
  MessageChannel = s.MessageChannel = s.com_github_osblinnikov_cnets_types.MessageChannel = ->
    slf = this
    slf.port1 = new s.com_github_osblinnikov_cnets_types.MessagePort(undefined)
    slf.port2 = new s.com_github_osblinnikov_cnets_types.MessagePort(slf.port1)
    true


if s.Worker and !s.com_github_osblinnikov_cnets_types.MessageChannelArtificial
  s.com_github_osblinnikov_cnets_types.Worker = s.Worker
else
  Worker = s.Worker = s.com_github_osblinnikov_cnets_types.Worker = (scriptFile) ->
    slf = this
...
Dispatcher = s.Dispatcher = s.com_github_osblinnikov_cnets_types.Dispatcher = (binds)->
...
if isNode
  module.exports = s.com_github_osblinnikov_cnets_types
