...
describe "MessageChannel-API", ->
  it "should define the used API", ->
    expect(s.com_github_osblinnikov_cnets_types.MessageChannel).toEqual jasmine.any(Function)
...
describe "MessageChannel-send", ->
  it "should send the given message", ->
    ...
    ch = new s.com_github_osblinnikov_cnets_types.MessageChannel()
    wrk1 = new s.com_github_osblinnikov_cnets_types.Worker("/test/specs/msgch.sender.js")

    wrk1.postMessage("",[ch.port1])
    wrk2 = new s.com_github_osblinnikov_cnets_types.Worker("/test/specs/msgch.receiver.js")
