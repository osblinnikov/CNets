...
void com_github_airutech_cnets_selector_initialize(com_github_airutech_cnets_selector *that){
  com_github_airutech_cnets_selector_onCreate(that);
}
...
void com_github_airutech_cnets_selector_onCreate(com_github_airutech_cnets_selector *that){
  ...
  for(i=0; i<(int)that->reducableReaders.length; i++){
    ...
    linkedContainer_create(_linkedContainer_,com_github_airutech_cnets_selector_getWriter(that, NULL, i))
...
bufferReadData com_github_airutech_cnets_selector_readNextWithMeta(bufferKernelParams *params, BOOL make_timeout) {
  ...
  res.data = NULL;
  if(params == NULL){
    printf("ERROR: com_github_airutech_cnets_selector readNextWithMeta: params is NULL\n");
    return res;
  }
  com_github_airutech_cnets_selector *that = (com_github_airutech_cnets_selector*)params->target;
  if(that == NULL){
  ...
  if(that->sumWrites <= 0 && nanosec > 0){
    do{
      if(ETIMEDOUT == pthread_cond_timedwait(&that->switch_cv, &that->switch_cv_lock, &wait_timespec)){
        printf("ERROR: com_github_airutech_cnets_selector_readNextWithMeta: wait timeout, params->grid_id='%d'\n",params->grid_id);
      }
      if(that->sumWrites > 0){
        break;
...
int com_github_airutech_cnets_selector_readFinished(bufferKernelParams *params) {
  int res = -1;
  if(params == NULL){
    printf("ERROR: com_github_airutech_cnets_selector readFinished: params is NULL\n");
    return res;
  }
  com_github_airutech_cnets_selector *that = (com_github_airutech_cnets_selector*)params->target;
  if(that == NULL){
    printf("ERROR: com_github_airutech_cnets_selector readFinished: Some Input parameters are wrong\n");
    return res;
  }
  com_github_airutech_cnets_selector_container *container = (com_github_airutech_cnets_selector_container *)params->additionalData;
  if(container != NULL && container->bufferId >=0 && container->bufferId < (int)that->reducableReaders.length){
    ...
    printf("ERROR: com_github_airutech_cnets_selector readFinished: some params are wrong\n");
  }
  return res;
}
...
void* com_github_airutech_cnets_selector_writeNext(bufferKernelParams *params, BOOL make_timeout) {
  ...
  void* res = NULL;
  if(that == NULL){
    printf("ERROR: com_github_airutech_cnets_selector writeNext: Some Input parameters are wrong\n");
    return res;
  }
  printf("ERROR: com_github_airutech_cnets_selector writeNext is not allowed\n");
  return res;
}

int com_github_airutech_cnets_selector_writeFinished(bufferKernelParams *params) {
  if(params == NULL){
    printf("ERROR: com_github_airutech_cnets_selector writeFinished: params is NULL\n");
    return -1;
  }
  com_github_airutech_cnets_selector *that = (com_github_airutech_cnets_selector*)params->target;
  if(that == NULL){
...
int com_github_airutech_cnets_selector_size(bufferKernelParams *params){
  if(params == NULL){
    printf("ERROR: com_github_airutech_cnets_selector size: params is NULL\n");
    return -1;
  }
  com_github_airutech_cnets_selector *that = (com_github_airutech_cnets_selector*)params->target;
  if(that == NULL){
    printf("ERROR: com_github_airutech_cnets_selector size: Some Input parameters are wrong\n");
    return -1;
  };
  com_github_airutech_cnets_selector_container *container = (com_github_airutech_cnets_selector_container *)params->additionalData;
  if(container == NULL || container->bufferId <0 || container->bufferId >= (int)that->reducableReaders.length){
...
int com_github_airutech_cnets_selector_timeout(bufferKernelParams *params){
  ...
  com_github_airutech_cnets_selector *that = (com_github_airutech_cnets_selector*)params->target;
  if(that == NULL){
    printf("ERROR: com_github_airutech_cnets_selector timeout: Some Input parameters are wrong\n");
    return -1;
  };
...
int com_github_airutech_cnets_selector_addSelector(bufferKernelParams *params, void* selectorContainer) {
  printf("ERROR: com_github_airutech_cnets_selector addSelector is not allowed: nested selectors look ambiguous\n");
  return -1;
}

void com_github_airutech_cnets_selector_onKernels(com_github_airutech_cnets_selector *that){
  /*do nothing*/
  return;
}